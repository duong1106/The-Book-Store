...
<nav class="navbar navbar-expand-md bg-info">
    <div class="container">
        <a class="navbar-brand text-warning" href="MainController?action=home">BOOK STORE</a>
        ...
        <div class="collapse navbar-collapse justify-content-end" id="navbarsExampleDefault">
            <ul class="navbar-nav m-auto">
                <c:if test="${sessionScope.USER.getRoleID() == 1}">
                    <li class="nav-item">
                        <a class="nav-link text-warning" href="MainController?action=manager">Manager Book</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-warning" href="MainController?action=discount">Create Discount</a>
                    </li>
                </c:if>
                <c:if test="${sessionScope.USER != null}">
                    <li class="nav-item">
                        <a class="nav-link text-light" >Hello ${sessionScope.USER.getUserName()}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-warning" href="MainController?action=History">History Order</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-warning" href="MainController?action=Logout">Logout</a>
                    </li> 
                </c:if>
            </ul>
